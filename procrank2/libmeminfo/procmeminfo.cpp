...
// Converts MemUsage stats from KB to B in case usage is expected in bytes.
        ...
        bool ProcMemInfo::ResetWorkingSet(pid_t pid)
        {
            ...
            if (!::android::base::WriteStringToFile("1\n", clear_refs_path)) {
                LOG_ERROR("Failed to write to %s", clear_refs_path.c_str());
                return false;
            }
        ...
        const std::vector<Vma> &ProcMemInfo::Maps()
        {
            if (maps_.empty() && !ReadMaps(get_wss_)) {
                LOG_ERROR("Failed to read maps for Process %d", pid_);
            }
        ...
        const std::vector<Vma> &ProcMemInfo::MapsWithPageIdle()
        {
            if (maps_.empty() && !ReadMaps(get_wss_, true)) {
                LOG_ERROR("Failed to read maps with page idle for Process %d", pid_);
            }
        ...
        const std::vector<Vma> &ProcMemInfo::Smaps(const std::string &path, bool collect_usage)
        {
            ...
            if (path.empty() && !ForEachVma(collect_vmas)) {
                LOG_ERROR("Failed to read smaps for Process %d", pid_);
                maps_.clear();
            }

            if (!path.empty() && !ForEachVmaFromFile(path, collect_vmas)) {
                LOG_ERROR("Failed to read smaps from file %s", path.c_str());
                maps_.clear();
            }
        ...
        {
            if (get_wss_) {
                LOG_WARN("Trying to read process memory usage for %d using invalid object", pid_);
                return usage_;
            }

            if (maps_.empty() && !ReadMaps(get_wss_)) {
                LOG_ERROR("Failed to get memory usage for Process %d", pid_);
            }
        ...
        {
            if (!get_wss_) {
                LOG_WARN("Trying to read process working set for %d using invalid object", pid_);
                return usage_;
            }

            if (maps_.empty() && !ReadMaps(get_wss_)) {
                LOG_ERROR("Failed to get working set for Process %d", pid_);
            }
        ...
        {
            if (get_wss_) {
                LOG_WARN("Trying to read process swap offsets for %d using invalid object", pid_);
                return swap_offsets_;
            }

            if (maps_.empty() && !ReadMaps(get_wss_, false, true, true)) {
                LOG_ERROR("Failed to get swap offsets for Process %d", pid_);
            }
        ...
        bool ProcMemInfo::PageMap(const Vma &vma, std::vector<uint64_t> *pagemap)
        {
            ...
            if (pagemap_fd.get() == -1) {
                LOG_ERROR("Failed to open %s", pagemap_file.c_str());
                return false;
            }
            ...
            ssize_t bytes_read = pread64(pagemap_fd.get(), pagemap->data(), bytes_to_read, start_addr);
            if (bytes_read == -1) {
                LOG_ERROR("Failed to read page frames from page map for pid: %d", pid_);
                return false;
            } else if (static_cast<size_t>(bytes_read) != bytes_to_read) {
                LOG_ERROR("Failed to read page frames from page map for pid %d: read bytes %zd, expected bytes %zu",
                          pid_, bytes_read, bytes_to_read);
                return false;
            }
        ...
        bool ProcMemInfo::ReadMaps(bool get_wss, bool use_pageidle, bool get_usage_stats, bool swap_only)
        {
            ...
            if (!::android::procinfo::ReadMapFile(
                ...
                LOG_ERROR("Failed to parse %s", maps_file.c_str());
                maps_.clear();
                return false;
            }
        ...
        bool ProcMemInfo::GetUsageStats(bool get_wss, bool use_pageidle, bool swap_only)
        {
            ...
            for (auto &vma: maps_) {
                if (!ReadVmaStats(pagemap_fd.get(), vma, get_wss, use_pageidle, swap_only)) {
                    LOG_ERROR("Failed to read page map for vma %s [%lu-%lu]", vma.name.c_str(), vma.start, vma.end);
                    return false;
                }
                add_mem_usage(&usage_, vma.usage);
            }
                                       ...
                                       bool swap_only)
        {
            ...
            if (get_wss && use_pageidle && !pinfo.InitPageAcct(true)) {
                LOG_ERROR("Failed to init idle page accounting");
                return false;
            }
            ...
            for (size_t cur_page = first_page; cur_page < first_page + num_pages; ++cur_page) {
                ...
                if (cur_page_cache_index == num_in_page_cache) {
                    ...
                    if ((size_t) bytes != total_bytes) {
                        if (bytes == -1) {
                            LOG_ERROR("Failed to read page data at offset 0x%lx", cur_page * sizeof(uint64_t));
                        } else {
                            LOG_ERROR("Failed to read page data at offset 0x%lx. read bytes %zd, expected bytes %zu",
                                      cur_page * sizeof(uint64_t), bytes, total_bytes);
                        }
                        return false;
                    }
                ...
                if (!pinfo.PageFlags(page_frame, &cur_page_flags)) {
                    LOG_ERROR("Failed to get page flags for %lu in process %d", page_frame, pid_);
                    swap_offsets_.clear();
                    return false;
                }
                ...
                if (!pinfo.PageMapCount(page_frame, &cur_page_counts)) {
                    LOG_ERROR("Failed to get page count for %lu in process %d", page_frame, pid_);
                    swap_offsets_.clear();
                    return false;
                }
...
// Public APIs
                                ...
                                bool read_smaps_fields)
        {
            ...
            while ((line_len = getline(&line, &line_alloc, fp.get())) > 0) {
                ...
                if (!::android::procinfo::ReadMapFileContent(
                    ...
                    free(line);
                    LOG_ERROR("Failed to parse %s", path.c_str());
                    return false;
                }
        ...
        enum smaps_rollup_support
        {
            UNTRIED, SUPPORTED, UNSUPPORTED
        };
        ...
        bool IsSmapsRollupSupported()
        {
            ...
            LOG_INFO("Using smaps_rollup for pss collection");
            return true;
        }
