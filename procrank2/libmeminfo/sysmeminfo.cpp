...
namespace android
{
    namespace meminfo
    {
        std::initializer_list<std::string_view> SysMemInfo::kDefaultSysMemInfoTags = {
                                     ...
                                     const char *path)
        {
            return ReadMemInfo(path, ntags, tags, [&]([[maybe_unused]] std::string_view tag, uint64_t val)
            {
                ...
                if (it == tags + ntags) {
                    LOG_ERROR("Tried to store invalid tag: %s", tag.data());
                    return;
                }
            ...
            while (*p && found < ntags) {
                for (size_t tagno = 0; tagno < ntags; ++tagno) {
                    ...
                    if (strncmp(p, tag.data(), tag.size()) == 0) {
                        ...
                        if (p == endptr) {
                            LOG_ERROR("Failed to parse line: %d in file %s", lineno + 1, path);
                            return false;
                        }
                        store_val(tag, val);
                        p = endptr;
        ...
        bool SysMemInfo::MemZramDevice(const char *zram_dev, uint64_t *mem_zram_dev) const
        {
            ...
            if (mmstat_fp != nullptr) {
                ...
                if (fscanf(mmstat_fp.get(), "%*" SCNu64 " %*" SCNu64 " %" SCNu64, mem_zram_dev) != 1) {
                    LOG_ERROR("Malformed mm_stat file in: %s", zram_dev);
                    return false;
                }
                return true;
            }
            ...
            if (::android::base::ReadFileToString(
                ...
                *mem_zram_dev = strtoull(content.c_str(), NULL, 10);
                if (*mem_zram_dev == ULLONG_MAX) {
                    LOG_ERROR("Malformed mem_used_total file for zram dev: %s. content: %s", zram_dev, content.c_str());
                    return false;
                }
            ...
            LOG_ERROR("Can't find memory status under: %s", zram_dev);
            return false;
        }
...
// Public methods
        ...
        static bool ReadSysfsFile(const std::string &path, uint64_t *value)
        {
            ...
            *value = strtoull(content.c_str(), NULL, 10);
            if (*value == ULLONG_MAX) {
                LOG_ERROR("Invalid file format: %s", path.c_str());
                return false;
            }
        ...
        bool ReadDmabufHeapPoolsSizeKb(uint64_t *size, const std::string &dma_heap_pool_size_path)
        {
            static bool support_dmabuf_heap_pool_size = [dma_heap_pool_size_path]() -> bool
            {
                ...
                if (!ret)
                    LOG_ERROR("Unable to read DMA-BUF heap total pool size, read ION total pool size instead.");
                return ret;
            }();
                                           ...
                                           const std::string &dmabuf_sysfs_stats_path)
        {
            static bool support_dmabuf_heaps = [dma_heap_root_path]() -> bool
            {
                ...
                LOG_ERROR("DMA-BUF heaps not supported, read ION heap total instead.");
                return ret;
            }();
