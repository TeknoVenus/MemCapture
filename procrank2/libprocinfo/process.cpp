...
namespace android
{
    namespace procinfo
    {
        ...
        {
            android::base::unique_fd status_fd(openat(fd, "status", O_RDONLY | O_CLOEXEC));
            if (status_fd.get() == -1) {
                return SetError(error, errno,
                                "failed to open /proc/%d/status in GetProcessInfoFromProcPidFd", pid);
            }

            std::unique_ptr<FILE, decltype(&fclose)> fp(fdopen(status_fd.get(), "re"), fclose);
            if (!fp) {
                return SetError(error, errno, "failed to open FILE for /proc/%d/status in GetProcessInfoFromProcPidFd",
                                pid);
            }
            ...
            android::base::unique_fd stat_fd(openat(fd, "stat", O_RDONLY | O_CLOEXEC));
            if (stat_fd.get() == -1) {
                return SetError(error, errno, "failed to open /proc/%d/stat", pid);
            }

            std::string stat;
            if (!android::base::ReadFdToString(stat_fd.get(), &stat)) {
