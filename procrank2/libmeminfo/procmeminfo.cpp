...
bool ProcMemInfo::PageMap(const Vma& vma, std::vector<uint64_t>* pagemap) {
    ...
    if (bytes_read == -1) {
        ...
        LOG_ERROR("Failed to read page frames from page map for pid %d: read bytes %zd, expected bytes %zu", pid_, bytes_read, bytes_to_read);
        return false;
    }
        ...
        if (cur_page_cache_index == num_in_page_cache) {
            ...
            if ((size_t)bytes != total_bytes) {
                if (bytes == -1) {
                    ...
                    LOG_ERROR("Failed to read page data at offset 0x%lx. read bytes %zd, expected bytes %zu", cur_page * sizeof(uint64_t), bytes, total_bytes);
                }
                return false;
            }
