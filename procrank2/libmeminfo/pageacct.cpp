    ...
    namespace meminfo
    {
        ...
        bool PageAcct::InitPageAcct(bool pageidle_enable)
        {
            if (pageidle_enable && !PageAcct::KernelHasPageIdle()) {
                LOG_ERROR("Idle page tracking is not supported by the kernel");
                return false;
            }

            if (kpagecount_fd_.get() < 0) {
                android::base::unique_fd count_fd(TEMP_FAILURE_RETRY(open("/proc/kpagecount", O_RDONLY | O_CLOEXEC)));
                if (count_fd.get() < 0) {
                    LOG_ERROR("Failed to open /proc/kpagecount");
                    return false;
                }
                kpagecount_fd_ = std::move(count_fd);
            }

            if (kpageflags_fd_.get() < 0) {
                android::base::unique_fd flags_fd(TEMP_FAILURE_RETRY(open("/proc/kpageflags", O_RDONLY | O_CLOEXEC)));
                if (flags_fd.get() < 0) {
                    LOG_ERROR("Failed to open /proc/kpageflags");
                    return false;
                }
                kpageflags_fd_ = std::move(flags_fd);
            }

            if (pageidle_enable && pageidle_fd_.get() < 0) {
                android::base::unique_fd idle_fd(TEMP_FAILURE_RETRY(open("/sys/kernel/mm/page_idle/bitmap", O_RDWR | O_CLOEXEC)));
                if (idle_fd.get() < 0) {
                    LOG_ERROR("Failed to open page idle bitmap");
                    return false;
                }
        ...
        bool PageAcct::PageFlags(uint64_t pfn, uint64_t *flags)
        {
            ...
            if (pread64(kpageflags_fd_.get(), flags, sizeof(uint64_t), pfn * sizeof(uint64_t)) !=
                sizeof(uint64_t)) {
                LOG_ERROR("Failed to read page flags for page %lu", pfn);
                return false;
            }
            return true;
        }
        ...
        bool PageAcct::PageMapCount(uint64_t pfn, uint64_t *mapcount)
        {
            ...
            if (pread64(kpagecount_fd_.get(), mapcount, sizeof(uint64_t), pfn * sizeof(uint64_t)) !=
                sizeof(uint64_t)) {
                LOG_ERROR("Failed to read map count for page %lu", pfn);
                return false;
            }
            return true;
        }
        ...
        int PageAcct::MarkPageIdle(uint64_t pfn) const
        {
            ...
            if (pwrite64(pageidle_fd_.get(), &idle_bits, sizeof(uint64_t), offset) < 0) {
                LOG_ERROR("Failed to write page idle bitmap for page %lu", pfn);
                return -errno;
            }
        ...
        int PageAcct::GetPageIdle(uint64_t pfn) const
        {
            ...
            if (pread64(pageidle_fd_.get(), &idle_bits, sizeof(uint64_t), offset) != sizeof(uint64_t)) {
                LOG_ERROR("Failed to read page idle bitmap for page %lu", pfn);
                return -errno;
            }
