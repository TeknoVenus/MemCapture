...
namespace android
{
    namespace meminfo
    {
        ...
        public:
            ...
            static std::initializer_list<std::string_view> kDefaultSysMemInfoTags;
            ...
            // getters
            uint64_t mem_total_kb() const
            { return find_mem_by_tag(kMemTotal); }

            uint64_t mem_free_kb() const
            { return find_mem_by_tag(kMemFree); }

            uint64_t mem_buffers_kb() const
            { return find_mem_by_tag(kMemBuffers); }

            uint64_t mem_cached_kb() const
            { return find_mem_by_tag(kMemCached); }

            uint64_t mem_shmem_kb() const
            { return find_mem_by_tag(kMemShmem); }

            uint64_t mem_slab_kb() const
            { return find_mem_by_tag(kMemSlab); }

            uint64_t mem_slab_reclaimable_kb() const
            { return find_mem_by_tag(kMemSReclaim); }

            uint64_t mem_slab_unreclaimable_kb() const
            { return find_mem_by_tag(kMemSUnreclaim); }

            uint64_t mem_swap_kb() const
            { return find_mem_by_tag(kMemSwapTotal); }

            uint64_t mem_swap_free_kb() const
            { return find_mem_by_tag(kMemSwapFree); }

            uint64_t mem_mapped_kb() const
            { return find_mem_by_tag(kMemMapped); }

            uint64_t mem_vmalloc_used_kb() const
            { return find_mem_by_tag(kMemVmallocUsed); }

            uint64_t mem_page_tables_kb() const
            { return find_mem_by_tag(kMemPageTables); }

            uint64_t mem_kernel_stack_kb() const
            { return find_mem_by_tag(kMemKernelStack); }

            uint64_t mem_kreclaimable_kb() const
            { return find_mem_by_tag(kMemKReclaimable); }

            uint64_t mem_active_kb() const
            { return find_mem_by_tag(kMemActive); }

            uint64_t mem_inactive_kb() const
            { return find_mem_by_tag(kMemInactive); }

            uint64_t mem_unevictable_kb() const
            { return find_mem_by_tag(kMemUnevictable); }

            uint64_t mem_available_kb() const
            { return find_mem_by_tag(kMemAvailable); }

            uint64_t mem_active_anon_kb() const
            { return find_mem_by_tag(kMemActiveAnon); }

            uint64_t mem_inactive_anon_kb() const
            { return find_mem_by_tag(kMemInactiveAnon); }

            uint64_t mem_active_file_kb() const
            { return find_mem_by_tag(kMemActiveFile); }

            uint64_t mem_inactive_file_kb() const
            { return find_mem_by_tag(kMemInactiveFile); }

            uint64_t mem_cma_total_kb() const
            { return find_mem_by_tag(kMemCmaTotal); }

            uint64_t mem_cma_free_kb() const
            { return find_mem_by_tag(kMemCmaFree); }
