    ...
    namespace dmabufinfo
    {
                                             ...
                                             const std::string &procfs_path)
        {
            ...
            if (!fp) {
                ...
                LOG_ERROR("Failed to open %s", fdinfo.c_str());
                return ERROR;
            }
...
// Public methods
                              ...
                              const std::string &procfs_path)
        {
            ...
            if (!dir) {
                ...
                if (errno != EACCES) {
                    LOG_ERROR("Failed to open %s directory", fdinfo_dir_path.c_str());
                } else if (!logged_permission_err) {
                    LOG_ERROR("%s", permission_err_msg);
            ...
            while ((dent = readdir(dir.get()))) {
                ...
                if (fdinfo_result != OK) {
                    ...
                    if (errno != EACCES) {
                        LOG_ERROR("Failed to read fd info for pid: %d, fd: %d", pid, fd);
                    } else if (!logged_permission_err) {
                        LOG_ERROR("%s", permission_err_msg);
                        logged_permission_err = true;
                    }
                ...
                if (inode == static_cast<uint64_t>(-1)) {
                    ...
                    if (stat(fd_path.c_str(), &sb) < 0) {
                        ...
                        LOG_ERROR("Failed to stat: %s", fd_path.c_str());
                        return false;
                    }
                ...
                auto buf = std::find_if(dmabufs->begin(), dmabufs->end(),
                                        [&inode](const DmaBuffer &dbuf)
                                        { return dbuf.inode() == inode; });
                if (buf != dmabufs->end()) {
                               ...
                               const std::string &dmabuf_sysfs_path)
        {
            ...
            if (!fp) {
                LOG_ERROR("Failed to open %s for pid %d", mapspath.c_str(), pid);
                return false;
            }
            ...
            for (std::string line; getline(fp, line);) {
                if (!::android::procinfo::ReadMapFileContent(line.data(), account_dmabuf)) {
                    LOG_ERROR("Failed to parse %s for pid %d", mapspath.c_str(), pid);
                    return false;
                }
            }
                            ...
                            const std::string &procfs_path, const std::string &dmabuf_sysfs_path)
        {
            ...
            if (read_fdrefs) {
                if (!ReadDmaBufFdRefs(pid, dmabufs, procfs_path)) {
                    LOG_ERROR("Failed to read dmabuf fd references");
                    return false;
                }
            }

            if (!ReadDmaBufMapRefs(pid, dmabufs, procfs_path, dmabuf_sysfs_path)) {
                LOG_ERROR("Failed to read dmabuf map references");
                return false;
            }
            return true;
        }
        ...
        bool ReadProcfsDmaBufs(std::vector<DmaBuffer> *bufs)
        {
            ...
            while ((dent = readdir(dir.get()))) {
                ...
                if (!ReadDmaBufFdRefs(pid, bufs)) {
                    LOG_ERROR("Failed to read dmabuf fd references for pid %d", pid);
                }

                if (!ReadDmaBufMapRefs(pid, bufs)) {
                    LOG_ERROR("Failed to read dmabuf map references for pid %d", pid);
                }
            }
