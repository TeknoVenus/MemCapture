    ...
    namespace dmabufinfo
    {
        ...
        static bool ReadUintFromFile(const std::string &path, uint64_t *val)
        {
            ...
            if (!android::base::ReadFileToString(path, &temp)) {
                LOG_ERROR("Unable to access %s", path.c_str());
                return false;
            }

            if (!android::base::ParseUint(android::base::Trim(temp).c_str(), val)) {
                LOG_ERROR("Unable to parse value from %s", path.c_str());
                return false;
            }
            return true;
        }
        ...
        bool GetDmabufSysfsStats(DmabufSysfsStats *stats, const std::string &dmabuf_sysfs_stats_path)
        {
            ...
            if (!dir) {
                LOG_ERROR("Unable to access: %s", dmabuf_sysfs_stats_path.c_str());
                return false;
            }
            ...
            *stats = {};
            ...
            while ((dent = readdir(dir.get()))) {
                ...
                if (!android::base::ParseUint(dent->d_name, &info.inode)) {
                    LOG_ERROR("Unable to parse value from %s", dent->d_name);
                    return false;
                }
                ...
                if (!android::base::ReadFileToString(exp_name_path, &exporter_name)) {
                    LOG_ERROR("Unable to access %s", exp_name_path.c_str());
                    return false;
                }
