...
/**
 * Output the results as a table in a single file - sent to stdout
 */
class TableReportGenerator : public IReportGenerator
{
public:
    TableReportGenerator(std::filesystem::path directory, std::string name, std::vector<std::string> columnNames);

    ~TableReportGenerator() override = default;
